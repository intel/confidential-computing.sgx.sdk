...
DEB_FOLDER=${SGX_HEADERS_PACKAGE_NAME}-${SGX_HEADERS_VERSION}

SGX_VERSION=$(awk '/STRFILEVER/ {print $3}' ${ROOT_DIR}/common/inc/internal/se_version.h|sed 's/^\"\(.*\)\"$/\1/')
DEB_BUILD_FOLDER=${SGX_HEADERS_PACKAGE_NAME}-${SGX_VERSION}
...
create_upstream_tarball() {
    ...
    cp ${LINUX_INSTALLER_COMMON_SGX_HEADERS_DIR}/output/${TARBALL_NAME} ${SCRIPT_DIR}
}
...
rename_tarball() {
    TARBALL_NAME_NEW_VERSION=$(echo ${TARBALL_NAME} | sed "s/${SGX_HEADERS_VERSION}/${SGX_VERSION}/")
